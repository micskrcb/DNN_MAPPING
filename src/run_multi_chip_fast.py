    ...
    class ReplayBuffer:
        ...
        def add(self, state, action, reward, next_state, done):
            if self._states is None:
                ...
                self._states = np.empty((self.capacity, state.size), dtype=np.float32)
                self._actions = np.empty((self.capacity, action.size), dtype=np.float32)
                self._rewards = np.empty(self.capacity, dtype=np.float32)
                self._next_states = np.empty((self.capacity, next_state.size), dtype=np.float32)
...
class MultiChipCoreMapper:
    def __init__(self, env: MultiChipEnvironment, baseline_latency: float = None, batch_z: int = 3):
        ...
        self._num_cores = self.total_rows * self.total_cols
        self._core_x = np.arange(self._num_cores, dtype=np.int32) % self.total_cols
        self._core_y = np.arange(self._num_cores, dtype=np.int32) // self.total_cols
        self._occ_values = np.zeros(self._num_cores, dtype=np.float32)
        self._task_comm = np.zeros(self.num_tasks, dtype=np.float32)
    ...
    def reset(self):
        self._placement.fill(-1)
        self._occupied.clear()
        self._task_ptr = 0
        self._occ_values.fill(0.0)
        self._task_comm.fill(0.0)
        self.env.reset()
        self._refresh_task_comm()
        return np.concatenate((self._occ_values, self._task_comm))

    def _refresh_task_comm(self):
        self._task_comm.fill(0.0)
        remaining = self.num_tasks - self._task_ptr
        n_batch = min(self.batch_z, remaining) if remaining > 0 else 0
        if n_batch:
            idx = self._task_ptr + np.arange(n_batch)
            self._task_comm[:] = (
                self.env.task_graph[idx].sum(axis=0) +
                self.env.task_graph[:, idx].sum(axis=1)
            )
        max_vol = self._task_comm.max()
        if max_vol > 0:
            self._task_comm /= max_vol
    ...
    def _place_one(self, target_x: float, target_y: float):
        ...
        if intended_core not in self._occupied:
            ...
            dist = (np.abs(self._core_x - intended_x) +
                    np.abs(self._core_y - intended_y)).astype(np.int32)
            if self._occupied:
                occupied_idx = np.fromiter(self._occupied, dtype=np.int32,
                                           count=len(self._occupied))
                dist[occupied_idx] = self._num_cores + 1
            core_id = int(np.argmin(dist))

        task = self._task_ptr
        self._placement[task] = core_id
        self._occupied.add(core_id)
        self._occ_values[core_id] = (task + 1) / self.num_tasks
...
def main():
    ...
    parser.add_argument("--baseline_trials", type=int, default=1000,
                        help="Random-search trials used once for DDPG baseline B")
